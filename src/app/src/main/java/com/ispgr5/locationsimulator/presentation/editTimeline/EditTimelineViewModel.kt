			...
			is EditTimelineEvent.ChangedSoundVolume -> {
				viewModelScope.launch {
					...
					when (component) {
						is Sound -> {
							component.minVolume =
								RangeConverter.transformPercentageToFactor(event.range.start)
							component.maxVolume =
								RangeConverter.transformPercentageToFactor(event.range.endInclusive)
						}
					}
				...
				viewModelScope.launch {
					...
					when (component) {
						is Vibration -> {
							...
							component.maxStrength =
								RangeConverter.floatTo8BitInt(event.range.endInclusive)
						}
					}
					compsCopy[state.value.currentTimelineIndex] = component
					_state.value = _state.value.copy(
